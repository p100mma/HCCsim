...
new_HCCSim_clustering_list<- function(x=list(),
				      domain_size=as.integer(length(unlist(x))),
				      params=list() )  {
stopifnot(is.list(x))
stopifnot(is.integer(domain_size))
stopifnot(is.list(params))
structure(x,
...
#' 
#' @param x A list of clusters. Each element is a vector of indexes of nodes that belong to each cluster. Names denote cluster labels. Whatever the names are in the list, they get converted to strings of consecutive integers starting from 1. List can exclude some nodes of originally clustered network, which makes them clusterless. 
...
for (j in seq_along(x_names)) names(x)[names(x)== x_names[[j]] ] = j
clist<- new_HCCSim_clustering_list(x=x,
				   domain_size=as.integer(domain_size),
				   params=params)
...
par_attr= attr(clustering_vector, "params")
if (is.null(par_attr)) par_attr= list()
HCCSim_clustering_list(init_list, domain_size= length(clustering_vector), params= par_attr )
}

#' Transform clustering_list to clustering_vector
#'
#' Transform a representation of clusters in terms of a list of indexes to the vector of numeric labels. See fifunctions \code{HCCSim_clustering_vector} and \code{HCCSim_clustering_list}.
#'
...
for (i in seq_along(clustering_list))
	init_vector[ clustering_list[[ as.character(i)]] ] = i
par_attr= attr(clustering_list, "params")
if (is.null(par_attr)) par_attr= list()
HCCSim_clustering_vector(init_vector, params=par_attr )
}
